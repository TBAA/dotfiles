...
" option g:rubycomplete_buffer_loading
...
fun! rubycomplete#LoadingAllowed()
  if !exists('g:rubycomplete_buffer_loading')
    let g:rubycomplete_buffer_loading = 'y' == input('enable rubycomplete_buffer_loading ? This evals Ruby code and may be insecure. [y/n]')
  end
  return g:rubycomplete_buffer_loading
endf
...
# {{{ ErrMsg
  def ErrMsg(msg)
    VIM::evaluate( "s:ErrMsg(\"%s\")" % msg.gsub(/([ "])/, "\\\\\\1") )
  end

# {{{ loading_allowed
  def loading_allowed
    VIM::evaluate("rubycomplete#LoadingAllowed()").to_i == 1
  end
# }}} loading_allowed
...
# {{{ buffer analysis magic
  ...
  def load_buffer_class(name)
    ...
    begin
      ...
      ErrMsg "Problem loading class \"%s\", was it already completed?" % name
    end
  ...
  def load_buffer_module(name)
    ...
    begin
      ...
      ErrMsg "Problem loading module \"%s\", was it already completed?" % name
    end
  ...
  def get_buffer_entity_list( type )
    ...
    return [] if allow_aggressive_load.to_i.zero? || !loading_allowed
  ...
  def load_rails
    ...
    if File.exists?( bootfile ) && File.exists?( envfile )
      begin
        ...
        ErrMsg "Error loading rails environment"
      end
    end
  end
